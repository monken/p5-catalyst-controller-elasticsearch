...
# ABSTRACT: Wrapper for ElasticSearch
...
sub model {
    ...
    if($model->can("esxmodel")) {
        ...
        $model = CatalystX::Controller::ElasticSearch::State->new(
            es => $model->$es,
            type => $self->type,
            index => $self->index,
        );
...
package CatalystX::Controller::ElasticSearch::State;
use Moose;
